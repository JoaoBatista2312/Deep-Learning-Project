...
class CNN(nn.Module):
    def __init__(self, dropout_prob, maxpool=True, batch_norm=True, conv_bias=True):
        ...
        self.conv_layers = nn.Sequential(                           #48x48x3
            ConvBlock(channels[0],channels[1], 3, 1, dropout=0.1,batch_norm=batch_norm),  #24x24x32
            ConvBlock(channels[1], channels[2], 3, 1, dropout=0.1,batch_norm=batch_norm), #12x12x64
            ConvBlock(channels[2], channels[3], 3, 1, dropout=0.1,batch_norm=batch_norm)  #6x6x128
        )
        ...
        if not batch_norm:
            ...
            self.MLP_layers = nn.Sequential(
                nn.Linear(channels[3]*6*6, out_features= fc1_out_dim),  
                nn.ReLU(),
        ...
        # For Q2.2 initalize batch normalization
            ...
            # Initialize layers for the MLP block
            self.MLP_layers = nn.Sequential(
                nn.Linear(channels[3]*1*1, out_features= fc1_out_dim),  
                nn.ReLU(),
                nn.BatchNorm1d(fc1_out_dim),
                nn.Dropout(dropout_prob),
                nn.Linear(in_features= fc1_out_dim,out_features=fc2_out_dim),
                nn.ReLU(),
                nn.Linear(in_features=fc2_out_dim,out_features=6)
    ...
    def forward(self, x):
        ...
        x = self.flatten(x)
        #print("Shape after adaptiveAvgPool2d:", x.shape)
        # Implement MLP part
        x = self.MLP_layers(x)
